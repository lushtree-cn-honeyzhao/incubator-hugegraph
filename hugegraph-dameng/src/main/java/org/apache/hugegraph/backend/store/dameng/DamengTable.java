...
public abstract class DamengTable
    ...
    private static final Logger LOG = Log.logger(DamengTable.class);
    ...
    private final DamengShardSplitter shardSplitter;

    public DamengTable(String table) {
        super(table);
        ...
        this.shardSplitter = new DamengShardSplitter(this.table());
    }
    ...
    @Override
    public void init(DamengSessions.Session session) {
        this.createTable(session, this.tableDefine());
    }

    @Override
    public void clear(DamengSessions.Session session) {
        this.dropTable(session);
    }

    public void truncate(DamengSessions.Session session) {
        this.truncateTable(session);
    }

    protected void createTable(DamengSessions.Session session, TableDefine tableDefine) {
    ...
    //TODO 达梦v7、v8 不需要 engine
    protected String engine(DamengSessions.Session session) {
        String engine = session.config().get(DamengOptions.JDBC_STORAGE_ENGINE);
        return " ENGINE=" + engine;
    }

    protected void dropTable(DamengSessions.Session session) {
    ...
    protected void truncateTable(DamengSessions.Session session) {
    ...
    protected List<Long> idColumnValue(DamengBackendEntry.Row entry) {
    ...
    protected void insertOrUpdate(DamengSessions.Session session, String template,
                                  List<?> params) {
    ...
    protected final String buildUpdateTemplate(DamengBackendEntry.Row entry) {
        if (entry.ttl() != 0L) {
    ...
    protected String buildUpdateForcedTemplate(DamengBackendEntry.Row entry) {
        StringBuilder insert = new StringBuilder();
        insert.append("REPLACE INTO ").append(this.table());
        return this.buildInsertKeys(insert, entry);
    }

    protected String buildUpdateIfAbsentTemplate(DamengBackendEntry.Row entry) {
        StringBuilder insert = new StringBuilder();
    ...
    protected List<?> buildUpdateForcedParams(DamengBackendEntry.Row entry) {
        return this.buildColumnsParams(entry);
    }

    protected List<?> buildUpdateIfAbsentParams(DamengBackendEntry.Row entry) {
        return this.buildColumnsParams(entry);
    }

    protected List<Object> buildColumnsParams(DamengBackendEntry.Row entry) {
    ...
    protected String buildUpdateIfPresentTemplate(DamengBackendEntry.Row entry) {
    ...
    protected List<?> buildUpdateIfPresentParams(DamengBackendEntry.Row entry) {
        List<HugeKeys> idNames = this.idColumnName();
     ...
     */
    @Override
    public void insert(DamengSessions.Session session, DamengBackendEntry.Row entry) {
    ...
    @Override
    public void delete(DamengSessions.Session session, DamengBackendEntry.Row entry) {
        List<HugeKeys> idNames = this.idColumnName();
    ...
    @Override
    public void append(DamengSessions.Session session, DamengBackendEntry.Row entry) {
        this.insert(session, entry);
    }

    @Override
    public void eliminate(DamengSessions.Session session, DamengBackendEntry.Row entry) {
        this.delete(session, entry);
    }

    @Override
    public void updateIfPresent(DamengSessions.Session session, DamengBackendEntry.Row entry) {
    ...
    @Override
    public void updateIfAbsent(DamengSessions.Session session, DamengBackendEntry.Row entry) {
    ...
    @Override
    public boolean queryExist(DamengSessions.Session session, DamengBackendEntry.Row entry) {
    ...
    @Override
    public Number queryNumber(DamengSessions.Session session, Query query) {
    ...
    @Override
    public Iterator<BackendEntry> query(DamengSessions.Session session, Query query) {
        return this.query(session, query, this::results2Entries);
    }

    protected <R> Iterator<R> query(DamengSessions.Session session, Query query,
    ...
    protected StringBuilder queryByRange(ConditionQuery query,
        ...
        if (DamengShardSplitter.START.equals(shard.start()) &&
            DamengShardSplitter.END.equals(shard.end()) &&
        ...
        if (page != null && !page.isEmpty()) {
            ...
            if (!DamengShardSplitter.END.equals(shard.end())) {
                where.and();
    ...
    protected void wrapPage(StringBuilder select, Query query, boolean scan) {
        ...
        if (!page.isEmpty()) {
            ...
            Map<HugeKeys, Object> columns = DamengEntryIterator.PagePosition.fromBytes(position)
                                                        .columns();
    ...
    protected Iterator<BackendEntry> results2Entries(Query query,
        ...
        return new DamengEntryIterator(results, query, this::mergeEntries);
    }
    ...
    private static class DamengShardSplitter extends ShardSplitter<DamengSessions.Session> {
        ...
        public DamengShardSplitter(String table) {
            super(table);
        }

        @Override
        public List<Shard> getSplits(DamengSessions.Session session, long splitSize) {
        ...
        @Override
        protected long estimateDataSize(DamengSessions.Session session) {
            return 0L;
        }

        @Override
        protected long estimateNumKeys(DamengSessions.Session session) {
            return 0L;
        }
    }
}
