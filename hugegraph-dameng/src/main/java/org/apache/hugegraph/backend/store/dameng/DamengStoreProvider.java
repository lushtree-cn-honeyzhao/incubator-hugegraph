...
import org.apache.hugegraph.backend.store.dameng.DamengStore.DamengGraphStore;
import org.apache.hugegraph.backend.store.dameng.DamengStore.DamengSchemaStore;
import org.apache.hugegraph.backend.store.dameng.DamengStore.DamengSystemStore;
import org.apache.hugegraph.config.HugeConfig;

public class DamengStoreProvider extends AbstractBackendStoreProvider {
    ...
    protected BackendStore newSchemaStore(HugeConfig config, String store) {
        return new DamengSchemaStore(this, this.database(), store);
    }

    @Override
    protected BackendStore newGraphStore(HugeConfig config, String store) {
        return new DamengGraphStore(this, this.database(), store);
    }

    @Override
    protected BackendStore newSystemStore(HugeConfig config, String store) {
        return new DamengSystemStore(this, this.database(), store);
    }
