...
public class DamengSerializer extends TableSerializer {
    ...
    public DamengSerializer(HugeConfig config) {
        super(config);
    }

    @Override
    public DamengBackendEntry newBackendEntry(HugeType type, Id id) {
        return new DamengBackendEntry(type, id);
    }

    @Override
    protected TableBackendEntry newBackendEntry(TableBackendEntry.Row row) {
        return new DamengBackendEntry(row);
    }

    @Override
    protected DamengBackendEntry convertEntry(BackendEntry backendEntry) {
        if (!(backendEntry instanceof DamengBackendEntry)) {
            throw new BackendException("Not supported by DamengSerializer");
        }
        return (DamengBackendEntry) backendEntry;
    }
    ...
    protected void writeUserdata(SchemaElement schema,
        ...
        assert entry instanceof DamengBackendEntry;
